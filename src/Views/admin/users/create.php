    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 md:p-8">
        ...
        <form method="POST" action="/admin/users/store">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div><label class="block text-sm font-medium mb-1">Имя *</label><input type="text" name="name" value="<?= htmlspecialchars($old['name'] ?? '') ?>" required class="w-full px-3 py-2 border rounded-lg dark:bg-gray-700"></div>
                <div><label class="block text-sm font-medium mb-1">Отчество *</label><input type="text" name="patronymic" value="<?= htmlspecialchars($old['patronymic'] ?? '') ?>" required class="w-full px-3 py-2 border rounded-lg dark:bg-gray-700"></div>
